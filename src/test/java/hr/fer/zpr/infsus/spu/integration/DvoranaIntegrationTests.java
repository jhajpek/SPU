...
public class DvoranaIntegrationTests {
	...
	public void DvoranaTestClient_GetDvoraneWithSearchQuery_ReturnListView() {
		webTestClient.get().uri("/dvorane?query=ijek").exchange().expectStatus().isOk().expectBody(String.class)
				.value(html -> {
					Assertions.assertThatCharSequence(html).contains("Popis dvorana");
					Assertions.assertThatCharSequence(html).contains("Dodaj dvoranu");
					Assertions.assertThatCharSequence(html).contains("Rijeka");
	...
	public void DvoranaTestClient_GetDvoranaForm_ReturnFormView() {
		webTestClient.get().uri("/dvorane/new").exchange().expectStatus().isOk().expectBody(String.class)
				.value(html -> {
	...
	public void DvoranaTestClient_SaveDvorana_ReturnListView() {
		...
		webTestClient.post().uri("/dvorane").contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.body(BodyInserters.fromFormData("naziv", "Dvorana 3").with("lokacijaId", lokacijaId.toString()))
	...
	public void DvoranaTestClient_SaveInvalidDvorana_ReturnListView() {
		...
		webTestClient.post().uri("/dvorane").contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.body(BodyInserters.fromFormData("naziv", "").with("lokacijaId", lokacijaId.toString())).exchange()
				.expectStatus().isOk().expectBody(String.class).value(html -> {
					Assertions.assertThat(html).contains("Spremi");
				});
	}
	...
	public void DvoranaTestClient_GetEditForm_ReturnFormView() {
		...
		webTestClient.get().uri("/dvorane/edit/" + dvorana.getDvoranaId()).exchange().expectStatus().isOk()
	...
	public void DvoranaTestClient_UpdateDvorana_ReturnListView() {
		...
		webTestClient.post().uri("/dvorane/edit/" + dvorana.getDvoranaId())
				.contentType(MediaType.APPLICATION_FORM_URLENCODED)
				.body(BodyInserters.fromFormData("naziv", "Dvorana 0").with("lokacijaId", lokacijaId.toString()))
				.exchange().expectStatus().is3xxRedirection();
	...
	public void DvoranaTestClient_DeleteDvorana_ReturnListView() {
		...
		webTestClient.post().uri("/dvorane/delete/" + dvoranaId).exchange().expectStatus().is3xxRedirection();
